...
NW_FORMAT = {
  ...
  10: [['name', str, True],  [None, None, False],    ['name', str, True],      ["dist", float, True]], # Flexible with internal node names
...
def read_newick(newick, root_node=None, format=1):
...
def _read_newick_from_string(nw, root_node, format):
    ...
    for chunk in nw.split("(")[1:]:
        ...
        subchunks = list(map(lambda x:x.strip(), chunk.split(",")))
