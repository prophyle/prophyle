...
if GITDIR:
    ...
    PROPAGATION_POSTPROCESSING = os.path.join(
        C_D, "prophyle_propagation_postprocessing.py")
    PROPAGATION_PREPROCESSING = os.path.join(
        C_D, "prophyle_propagation_preprocessing.py")
...
def _test_tree(fn):
    ...
    assert pro.validate_prophyle_nhx_tree(
...
def _add_configuration_parameter(parser, visible=True):
    parser.add_argument(
        ...
        if visible else argparse.SUPPRESS, )
...
def _missing_library(d):
    ...
    if _is_complete(d, 1):
        pro.message(
            "Skipping downloading library '{}' (already exists)".format(l))
        return False
    else:
...
def prophyle_download(library, library_dir, force=False):
    ...
    if lib_missing or force:
        ...
        pro.test_files(nhx)
        pro.message(
            "Copying Newick/NHX tree '{}' to '{}'".format(nhx, new_nhx))
    ...
    if library == 'bacteria':
        if lib_missing or force:
            cmd = [
                'cd', d, '&&', 'curl', FTP_NCBI +
                '/genomes/archive/old_refseq/Bacteria/all.fna.tar.gz', '|',
                'tar', 'xz'
            ]
            pro.run_safe(cmd)
            _mark_complete(d, 1)
        ...
        if lib_missing or force:
            ...
            cmd = [
                'cd', d, '&&', 'curl',
                FTP_NCBI + '/genomes/Viruses/all.fna.tar.gz', '|', 'tar', 'xz'
            ]
            pro.run_safe(cmd)
            _mark_complete(d, 1)
        # _pseudo_fai(d)
        ...
        if lib_missing or force:
            cmd = [
                'cd', d, '&&', 'curl', FTP_NCBI +
                '/genomes/archive/old_refseq/Plasmids/plasmids.all.fna.tar.gz',
                '|', 'tar', 'xz', '--strip', '5'
            ]
            pro.run_safe(cmd)
            _mark_complete(d, 1)
...
def _propagate(index_dir, threads):
    ...
    propagation_dir = os.path.join(index_dir, 'propagation')
    pro.test_files(
        os.path.join(propagation_dir, 'Makefile'), test_nonzero=True)
...
def _kmer_stats(index_dir):
    ...
    propagation_dir = os.path.join(index_dir, 'propagation')
    command = [
        "cat", propagation_dir + "/*.count.tsv", "|", "grep", "-v", "^#", "|",
        "sort", "|", "uniq", ">",
...
def _propagation_postprocessing(index_dir, in_tree_fn, out_tree_fn):
    ...
    command = [
        PROPAGATION_POSTPROCESSING, propagation_dir, index_fa, in_tree_fn,
        tsv_fn, out_tree_fn
    ]
    pro.run_safe(
        command,
...
def _bwtocc2sa_klcp(fa_fn, k):
    ...
    pro.run_safe(
        command,
        err_msg=
        "Parallel construction of k-Longest Common Prefix array and Sampled Suffix Array failed.",
        thr_exc=True, )
    _log_file_md5(fa_fn + ".sa")
    _log_file_md5("{}.{}.klcp".format(fa_fn, k))


def prophyle_index(index_dir, threads, k, trees_fn, library_dir,
                   construct_klcp, force, no_prefixes, mask_repeats,
    ...
    if recompute:
        ...
        for tree_fn in trees_fn:
            ...
            if root != "":
                assert len(tree.search_nodes(name=root)
                           ) != 0, "Node '{}' does not exist in '{}'.".format(
                               root, tree_fn)
        if len(trees_fn) != 1:
        ...
        pro.message(
            '[1/6] Tree already exists, skipping its creation', upper=True)
    ...
    if recompute:
        ...
        pro.message(
            '[2/6] K-mers have already been propagated, skipping propagation',
            upper=True)
    ...
    if not _is_complete(index_dir, 3) and not _is_complete(
            index_dir, 4, dont_check_previous=True):
        recompute = True
    ...
    if recompute:
        ...
        pro.message(
            '[3/6] BWT already exists, skipping its construction', upper=True)
    ...
    if recompute:
        ...
        pro.message(
            '[4/6] OCC already exists, skipping their construction',
            upper=True)
    ...
    if construct_klcp:
        ...
        if recompute:
            pro.message(
                '[5/6],[6/6] Constructing SA + KLCP in parallel ', upper=True)
    ...
    if recompute:
        ...
        pro.message(
            '[5/6] SA already exists, skipping its construction', upper=True)
    ...
    if construct_klcp:
        ...
        if recompute:
            ...
            pro.message(
                '[6/6] k-LCP already exists, skipping its construction',
                upper=True)
...
def prophyle_classify(index_dir, fq_fn, fq_pe_fn, k, out_format, mimic_kraken,
                      measure, annotate, tie_lca, kmer_lca, print_seq, cimpl,
    ...
    if force_restarted_search:
        ...
        if use_rolling_window:
            ...
            assert abs(bwt_s -
                       4 * klcp_s) < 1000, 'Inconsistent index (KLCP vs. BWT)'
        else:
    ...
    cmd_query = [
        IND, 'query', '-k', k, '-u' if use_rolling_window else '', '-b'
        if print_seq else '', index_fa, in_read, '|'
    ]
...
def prophyle_decompress(archive, output_dir, klcp):
    ...
    with tarfile.open(archive) as tar:
        ...
        for x in FILES_TO_ARCHIVE:
            assert os.path.join(
                index_name,
                x) in names, "File '{}' is missing in the archive".format(x)
...
def parser():
    ...
    parser.add_argument(
        ...
        '--version',
        action='version',
        version='ProPhyle {} (rev {}, commit {})'.format(
            version.VERSION, version.REVCOUNT, version.SHORTHASH), )

    _add_configuration_parameter(parser, visible=False)

    subparsers = parser.add_subparsers(
        help="", description=argparse.SUPPRESS, dest='subcommand', metavar="")
    ...
    parser_classify.add_argument(
        ...
        choices=['h1', 'c1', 'h2', 'c2'],
        help=
        'measure: h1=hit count, c1=coverage, h2=norm.hit count, c2=norm.coverage [{}]'.
        format(DEFAULT_MEASURE),
    ...
    try:
        ...
        if subcommand == "download":
            ...
            prophyle_classify(
                ...
                prophyle_conf_string=
                prophyle_conf_string,  # already preprocessed
            )
            ...
            msg_lns = [
                x for x in msg_lns
                if x.find("optional arguments") == -1 and x.find("--") == -1
            ]
            msg = "\n".join(msg_lns)
            msg = msg.replace("\n\n", '\n').replace("subcommands:\n",
                                                    "Command:\n").replace(
