            ...
            for read in read_iterator:
                ...
                try:
                    if read_ref != 'merge_root':
                        current_asgs[read_name.strip()].append(
                            read_ref.strip())
                except KeyError:
            ...
            for tax in ref:
                if tax != "merge_root":
                    try:
                        ...
                        print(
                            "[prophyle_analyze] Node {} not found in the tree".
                            format(n1_name),
                            file=sys.stderr)
                        raise
                if n.is_leaf():
...
def print_histogram(histogram, out_f, tax_tree=None):
    ...
    for node_name, w in merged_histo.most_common():
        ...
        out_line = [node_name] + [
            "{:.2f}".format(round(c, 2)) if isinstance(c, float) else str(c)
            for c in sample_scores
        ]
        if tax_tree is not None:
...
def load_histo(in_fns, tree):
    ...
    for fn in in_fns:
        with open(fn, 'r') as f:
            ...
            if len(samples) > len(KNOWN_RANKS) and samples[-len(
            ...
            for line_num, line in enumerate(f):
                ...
                assert len(fields) == (
                    len(samples) +
                    1), "Malformed histogram (check fields at line {})".format(
                        line_num + 2)
                node_name = fields[0]
                scores = fields[1:]
                if not node_name in tree_names:
                    print(
                        """[prophyle_analyze] Error: node name {} found in the
                            histogram {} but not in the tree""".format(
                            n1_name, fn),
                        file=sys.stderr)
                    raise
            ...
            try:
                ...
                if '-' in node_name:
                    if taxid != node_name.split('-')[1]:
                        print(
                            """[prophyle_analyze] the taxid of node {} is different
                ...
                print(
                    """[prophyle_analyze] taxonomic annotations (taxid or
...
def compute_otu_table(histogram, tree):
    ...
    for sample, histo in histogram.items():
        ...
        if ncbi:
            ...
            print(
                "[prophyle_analyze] Missing some taxonomic tree annotations, switching to leaves-mode"
            )
            for node_name, count in histo.items():
                try:
                    ...
                    print(
                        "[prophyle_analyze] Error: node {} not in the tree".
                        format(node_name),
                        file=sys.stderr)
                    raise
    ...
    for node in tree.traverse('preorder'):
        ...
        if node_name != 'merge_root':
            ...
            if count > 0:
                ...
                out_line.append("{:.2f}".format(round(count, 2)) if isinstance(
                    count, float) else str(count))
                # Number of reads assigned directly to this taxon
                out_line.append("{:.2f}".format(round(hits, 2)) if isinstance(
                    hits, float) else str(hits))
                # Rank code
...
def print_metaphlan_report(otu_table, tot_count, tree, out_f):
    ...
    for node in tree.traverse('preorder'):
        ...
        if sum(counts) > 0:
            ...
            # Percentage of reads covered by the clade rooted at this taxon
            out_line = [tax_string] + [
                "{:.5f}".format(round(c * 100 / tot_count, 5)) for c in counts
            ]
            print(*out_line, sep='\t', file=out_f)


def print_centrifuge_report(otu_table, histogram, unique_histogram, tree,
                            out_f):
    header = [
        '#name', 'taxID', 'taxRank', 'kmerCount', 'numReads', 'numUniqueReads',
        'abundance'
    ]
    print(*header, sep='\t', file=out_f)
    merged_histo = sum(histogram.values(), Counter())
    merged_unique_histo = sum(
        unique_histogram.values(),
        Counter()) if unique_histogram is not None else None
    merged_otu = sum(otu_table.values(), Counter())
    for node in tree.traverse('preorder'):
        node_name = node.name
        count = merged_otu[node_name]
        ...
        if count > 0:
            hits = merged_histo[node_name]
            unique_hits = merged_unique_histo[
                node_name] if merged_unique_histo is not None else '0'
            try:
                rank = node.rank
            except AttributeError:
                rank = 'unclassified'
            ...
            out_line.append("{:.8f}".format(round(hits / kmer_count, 8))
                            if kmer_count != 0 else 0)
            print(*out_line, sep='\t', file=out_f)
...
def main():
    ...
    if len(histo_fns) > 0:
        assert len(args.input_fns) == len(
            histo_fns
        ), "Mixed histogram/assignments input formats not supported"
    ...
    if ncbi:
        with open(args.out_prefix + '.kraken.tsv', 'w') as f:
            tot_count = print_kraken_report(otu_table, histogram, unclassified,
                                            tree, f)
        ...
        with open(args.out_prefix + '.centrifuge.tsv', 'w') as f:
            print_centrifuge_report(otu_table, histogram, unique_histogram,
                                    tree, f)
