            ...
            with open(fn, 'r') as faidx:
                for seq in faidx:
                    try:
                        ...
                        if log:
                            print(
                                '[prophyle_ncbi_tree] ERROR: Sequence \"' +
                                seqname + '\" in file \"' + rel_fn +
                                '\" not acquired',
                                file=log)
                        skipped += 1
                        pass
...
def assign_sequences(taxid_map_f, seqs, log):
    ...
    for acc, dic in seqs.items():
        try:
            ...
            if log:
                print(
                    '[prophyle_ncbi_tree] ERROR: TaxID not found for sequence '
                    + acc,
                    file=log)
            skipped += 1
            pass
    ...
    if log:
        ...
        for node in t.traverse('postorder'):
            if not node.is_leaf() and node.taxid in taxa:
                internal_with_fasta += len([
                    acc for acc in taxa2acc[node.taxid] if acc in seqs.keys()
                ])
        print(
            '[prophyle_ncbi_tree] ' + str(internal_with_fasta) + ' sequences' +
            ' ignored because associated to internal node (see issue #153)',
            file=log)
...
def main_fun(library, library_dir, output_f, taxid_map, red_factor, root,
             log_file):
    ...
    print(
        '[prophyle_ncbi_tree] Acquired ' + str(acquired) + ' sequences (' +
        str(skipped) + ' skipped)',
        file=sys.stderr)
    if log_file:
        print(
    ...
    print(
        '[prophyle_ncbi_tree] TaxID assigned to ' + str(assigned) +
        ' sequences (' + str(skipped) + ' skipped)',
        file=sys.stderr)
    if log_file:
        print(
            '[prophyle_ncbi_tree] TaxID assigned to ' + str(assigned) +
    ...
    tax_tree, seq_count, node_count = build_tree(seqs, taxa2seqid, red_factor,
                                                 root, log_file)

    print(
        '[prophyle_ncbi_tree] Built taxonomic tree for ' + str(seq_count) +
        ' sequences (' + str(node_count) + ' nodes)',
        file=sys.stderr)
    if log_file:
        print(
            '[prophyle_ncbi_tree] Built taxonomic tree for ' + str(seq_count) +
    ...
    tax_tree.write(
        features=[
            'name', 'accession', 'taxid', 'sci_name', 'fastapath',
            'infasta_offset', 'base_len', 'rank', 'lineage', 'named_lineage'
        ],
        format=1,
...
def main():
    ...
    parser.add_argument(
        ...
        metavar='<library>',
        help=
        'directory with the library sequences (e.g. bacteria, viruses etc.)', )
    parser.add_argument(
        'library_dir',
        type=str,
        metavar='<library_dir>',
    ...
    parser.add_argument(
        'output_file', type=str, metavar='<output_file>', help='output file')
    parser.add_argument(
        'taxid_map_file',
        type=str,
        metavar='<taxid_map>',
    ...
    parser.add_argument(
        ...
        dest='root',
        help=
        'root of the tree (e.g. Bacteria); will exclude sequences which are not its descendants'
    )

    args = parser.parse_args()

    main_fun(args.library, args.library_dir, args.output_file,
             args.taxid_map_file, args.red_factor, args.root, args.log_file)
