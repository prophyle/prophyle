...
class Assignment:
    ...
    def blocks_to_masks(self, kmer_blocks, kmer_lca):
        ...
        hitmasks_dict = collections.defaultdict(
            lambda: bitarray(hitmask_empty))
        covmasks_dict = collections.defaultdict(
            lambda: bitarray(covmask_empty))
        ...
        for (node_names, count) in kmer_blocks:
            ...
            if node_names != ["0"] and node_names != ["A"]:
                ...
                covmask_1block = self.bitarray_block(covmask_len,
                                                     count + self.k - 1, pos)
    ...
    def compute_assignments(self):
        ...
        self.ass_dict = {
            nodename: self.evaluate_single_assignment(nodename)
    ...
    def make_lca_from_winners(self):
        ...
        for tag in ['h1', 'hf', 'h2', 'c1', 'cf', 'c2']:
            ass[tag] = [
                self.ass_dict[nodename][tag] for nodename in self.max_nodenames
            ],
    ...
    def print_selected_assignments(self, form):
        ...
        if form == "sam":
            ...
            for tag_ii, nodename in enumerate(self.max_nodenames, 1):
                ...
                suffix_parts = [
                    "ii:i:{}".format(tag_ii), "is:i:{}".format(tag_is)
                ]
                if self.annotate:
                    suffix_parts.append(
                        self.tree_index.nodename_to_samannot[nodename])
    ...
    def print_sam_line(self, node_name, suffix):
        ...
        columns = [
            ...
            "0",  # PNEXT
            "0",  # TLEN
            self.krakline_parser.seq
            if self.krakline_parser.seq else "*",  # SEQ
            self.krakline_parser.qual
            if self.krakline_parser.qual else "*",  # QUAL
        ]
    ...
    def print_kraken_line(self, *nodenames):
        ...
        columns = [
            stat, self.krakline_parser.readname, krak_ass,
            str(self.krakline_parser.readlen), krakmers
        ]
    ...
    def diagnostics(self):
        ...
        print("---------------------", file=sys.stderr)
        print(
            "Alignment.hitmasks_dict: ",
            dict(self.hitmasks_dict),
            file=sys.stderr)
        print(
            "Alignment.covmasks_dict: ",
            dict(self.covmasks_dict),
            file=sys.stderr)
        print(
            "Alignment.ass_dict:      ", dict(self.ass_dict), file=sys.stderr)
...
class TreeIndex:
    ...
    def diagnostics(self):
        ...
        print(
            "TreeIndex.nodename_to_node:        ",
            self.nodename_to_node,
            file=sys.stderr)
        print(
            "TreeIndex.nodename_to_samannot:    ",
            self.nodename_to_samannot,
            file=sys.stderr)
        print(
            "TreeIndex.nodename_to_upnodenames: ",
            self.nodename_to_upnodenames,
            file=sys.stderr)
        print(
            "TreeIndex.nodename_to_kmercount:   ",
            self.nodename_to_kmercount,
            file=sys.stderr)
...
class KraklineParser():
    ...
    def diagnostics(self):
        ...
        print(
            "KraklineParser.kmer_blocks: ", self.kmer_blocks, file=sys.stderr)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Implementation of assignment algorithm')
    ...
    parser.add_argument(
        ...
        dest='measure',
        help=
        'measure: h1=hit count, c1=coverage, h2=norm.hit count, c2=norm.coverage [h1]',
    )
